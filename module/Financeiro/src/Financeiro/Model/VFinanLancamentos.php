...
class VFinanLancamentos extends Model {
    ...
    public function buscaListagem($data_inicio, $data_fim, $string, $categoria_id){
        ...
        $sql = "select * from {$this->tableName} where dtavencimento between '$data_inicio' and '$data_fim' and descricao like '%$string%' $categoria_sql order by dtavencimento";
        return $this->executeSql($sql);
    ...
    public function buscaListagemResumo($data_inicio, $data_fim, $string, $categoria_id){
        ...
        $sql = "SELECT 
                    (select ifnull(sum(valor),0) from {$this->tableName} where tipo='R' and situacao='A' and descricao like '%$string%' and dtavencimento between '$data_inicio' and '$data_fim' $categoria_sql) 'receita_receber',
                    (select ifnull(sum(valor),0) from {$this->tableName} where tipo='R' and situacao='P' and descricao like '%$string%' and dtavencimento between '$data_inicio' and '$data_fim' $categoria_sql) 'receita_caixa',

                    (select ifnull(sum(valor),0) from {$this->tableName} where tipo='D' and situacao='A' and descricao like '%$string%' and dtavencimento between '$data_inicio' and '$data_fim' $categoria_sql) 'despesa_pagar',
                    (select ifnull(sum(valor),0) from {$this->tableName} where tipo='D' and situacao='P' and descricao like '%$string%' and dtavencimento between '$data_inicio' and '$data_fim' $categoria_sql) 'despesa_paga',

                    ((select ifnull(sum(valor),0) from {$this->tableName} where tipo='R' and situacao='P' and descricao like '%$string%' and dtavencimento between '$data_inicio' and '$data_fim' $categoria_sql)- (select ifnull(sum(valor),0) from {$this->tableName} where tipo='D' and situacao='P' and descricao like '%$string%' and dtavencimento between '$data_inicio' and '$data_fim' $categoria_sql)) 'caixa_total',
